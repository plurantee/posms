...
public class ShopResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new shop, or with
     * status {@code 400 (Bad Request)} if the shop has already an ID.
    ...
    public ResponseEntity<Shop> partialUpdateShop(@PathVariable(value = "id", required = false) final Long id,
                                                  @RequestBody Shop shop)
        throws URISyntaxException {
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the shop, or with status {@code
     * 404 (Not Found)}.
     */
    @GetMapping("/shops/{id}")
