  ...
  <div>
    <h2 id="page-heading" data-cy="InventoryHeading">
      ...
      <div class="d-flex justify-content-end">
        ...
        <router-link :to="{ name: 'ClientInventoryCreate' }" custom v-slot="{ navigate }">
          <button
            @click="navigate"
            id="jh-create-entity"
        ...
        <b-form-file
          v-model="file"
          placeholder="Bulk Upload Inventory"
      ...
      <table class="table table-striped" aria-describedby="inventories">
        ...
        <tbody>
          <tr v-for="inventory in inventories" :key="inventory.id" data-cy="entityTable">
            <td>
              <router-link :to="{ name: 'ClientInventoryView', params: { inventoryId: inventory.id } }">{{ inventory.id }}</router-link>
            </td>
            <td>{{ inventory.sku }}</td>
            ...
            <td class="text-right">
              <div class="btn-group">
                <router-link :to="{ name: 'ClientInventoryView', params: { inventoryId: inventory.id } }" custom v-slot="{ navigate }">
                  <button @click="navigate" class="btn btn-info btn-sm details" data-cy="entityDetailsButton">
                    <font-awesome-icon icon="eye"></font-awesome-icon>
                    <span class="d-none d-md-inline">View</span>
                  </button>
                </router-link>
                <router-link :to="{ name: 'ClientInventoryEdit', params: { inventoryId: inventory.id } }" custom v-slot="{ navigate }">
